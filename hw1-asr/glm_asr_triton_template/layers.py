...
class EncoderMLP:
    ...
    def _forward_fused(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.fc1.weight.device != x.device:
            ...
            self._fc1_weight_t = None
            self._fc1_bias_padded = None
        if self.fc1.bias_param is not None and self.fc1.bias_param.device != x.device:
        ...
        use_bias_fused = self.bias_enabled and self.fc1.bias_param is not None
        bias_for_kernel = None
        if use_bias_fused:
            if N == N_pad:
                bias_for_kernel = self.fc1.bias_param
            else:
                if self._fc1_bias_padded is None or self._fc1_bias_padded.shape[0] != N_pad:
                    self._fc1_bias_padded = torch.zeros(
                        (N_pad,), dtype=torch.float32, device=x.device
                    )
                self._fc1_bias_padded.zero_()
                self._fc1_bias_padded[:N] = self.fc1.bias_param
                bias_for_kernel = self._fc1_bias_padded
        ...
        if (not use_bias_fused) and self.bias_enabled and self.fc1.bias_param is not None:
